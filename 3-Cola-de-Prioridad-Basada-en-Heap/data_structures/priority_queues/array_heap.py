...
class ArrayHeap(PriorityQueueBase):
  ...
  def add(self, key: Any, value: Any) -> None:
    ...
    self._data.append(self._Item(
        key, value))  # _Item es la clase definida en PriorityQueueBase
    self._upheap(len(self._data) -
                 1)  #El nuevo nodo es hoja y hay que dejarlo ordenado.
  ...
  def remove_min(self) -> Tuple[Any, Any]:
    ...
    self._swap(0,
               len(self._data) -
               1)  # Pone el mínimo valor al final de la lista.
  ...
  def _downheap(self, j: int) -> None:
    ...
    if self._has_left(j):  # si el nodo tiene hijo izquierdo
      ...
      if self._has_right(j):  # si tiene hijo derecho
        right = self._right(j)
        if self._data[right] < self._data[
            left]:  # si el derecho es menor que el izquierdo.
          small_child = right

      if self._data[small_child] < self._data[
          j]:  # si el menor es menor que el actual.
